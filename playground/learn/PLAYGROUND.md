...
Use **Try V16 adaptive media** in the header to open the ready-to-use monitoring demo. **Normal**, **Warning**, and **Critical** apply different gain, position, and opacity values to the compiled source. Press Play after changing state to compare the audio. The indicator grows during playback using existing temporal animation: the browser audio clock drives `SceneAt(t)` on the stable snapshot supplying the WAV. Pause, resume, seek, and replay synchronize the preview without changing runtime parameter semantics. **Explore all visual parameters** loads a rectangle example covering gain, x/y position, width, height, rotation, and opacity. The [V16 guide](runtime-parameters.md) explains the supported bindings and lifecycle.
