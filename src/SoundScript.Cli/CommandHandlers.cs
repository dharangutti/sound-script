...
public static partial class CommandHandlers
{
    ...
    {
        var analysis = SourceAnalysis.Load(args.Input, "video"); var preflight = VideoPreflight.Check(args, analysis);
        if (args.Has("check"))
        {
        ...
        try
        {
            ...
            TemporalVideoFrameRenderer.WritePpmFrames(plan, directory, preflight.Width, preflight.Height);
            File.WriteAllBytes(audio, TemporalAudioRenderer.RenderToWavBytes(analysis.Program!, analysis.Timeline!.Duration));
            FfmpegWebmExporter.EncodeAndVerify(preflight.Ffmpeg, directory, audio, args.Value("out")!, plan);
        }
    ...
    private static int WordbankNormalize(CliArguments args)
    {
        ...
        if (args.Has("all"))
        {
            ...
            foreach (var lemma in CorpusCatalog.GetLemmaKeys(locale)) { if (normalizer.Normalize(lemma, locale).Success) normalized++; else missing++; }
