...
public sealed class CliProductTests : IDisposable
{
    ...
    private static string BuildPath(string project, string file) => Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "../../../..", project, "bin", new DirectoryInfo(AppContext.BaseDirectory).Parent!.Name, "net8.0", file));
    ...
    { var result = Run("--version"); Assert.Equal(0, result.Code); Assert.Equal(VersionInfo.Number, result.Out.Trim()); Assert.Equal("11.1.0", result.Out.Trim()); }
