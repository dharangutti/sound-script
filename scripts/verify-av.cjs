...
(async()=>{
  if (process.argv.includes('--cli')) {
    for(const key of keys) {
      ...
      execFileSync('dotnet',[require('./cli-build-path.cjs'),'video',path.join(root,`examples/visual-${key}.ssv`),'--output',file,'--fps','24','--width','640','--height','360'],{stdio:'pipe',timeout:180000});
